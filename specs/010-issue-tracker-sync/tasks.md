...
- [ ] T001 Extend `MannostreeConfigSchema` with `IssueTrackerConfigSchema` in `src/config/schema.ts`
...
- [ ] T002 [P] Define core types for `IssueTrackerProvider`, `IssueRecord`, `WorktreeIssueAttachment`, `IssueTransitionResult`, `IssueCommentResult`, `IssueDriftSummary`, and `IssueTrackerHealthStatus` in `src/types/index.ts`
- [ ] T003 [P] Implement Zod validation schemas for `IssueRecordSchema` and `WorktreeIssueAttachmentSchema` in `src/metadata/schema.ts`
- [ ] T004 Implement `MetadataStore` methods for saving/loading `.mannostree/issues/<KEY>.json` in `src/metadata/store.ts`
- [ ] T005 Implement `IssueTrackerAdapter` interface and `IssueTrackerRegistry` in `src/issues/base.ts`
...
- [ ] T006 [P] [US1] Unit test for Jira REST API v3 adapter in `tests/unit/issue-jira.test.ts`
- [ ] T007 [P] [US1] Unit test for Linear GraphQL API adapter in `tests/unit/issue-linear.test.ts`
- [ ] T008 [P] [US1] Unit test for GitHub Issues REST API adapter in `tests/unit/issue-github.test.ts`
- [ ] T009 [P] [US1] Integration test for issue ingestion and `spawn --issue` in `tests/integration/issue-sync-lifecycle.test.ts`

### Implementation for User Story 1
- [ ] T010 [US1] Implement `JiraAdapter` (REST API v3, Basic Auth / PAT) in `src/issues/jira.ts`
- [ ] T011 [US1] Implement `LinearAdapter` (GraphQL API, API Key Auth) in `src/issues/linear.ts`
- [ ] T012 [US1] Implement `GitHubIssueAdapter` (REST API, Bearer Auth) in `src/issues/github.ts`
- [ ] T013 [US1] Implement `GenericIssueAdapter` (Webhook / REST payload) in `src/issues/generic.ts`
- [ ] T014 [US1] Implement `IssueSyncEngine.ingestIssue` and contract markdown scaffolder in `src/issues/engine.ts`
- [ ] T015 [US1] Register `mannostree issue ingest` in `src/cli/commands/issue.ts` and extend `mannostree spawn --issue` in `src/cli/commands/spawn.ts`
...
- [ ] T016 [P] [US2] Unit test for lifecycle transition mappings and idempotency in `tests/unit/issue-engine.test.ts`

### Implementation for User Story 2
- [ ] T017 [US2] Implement `transitionIssue` and lifecycle state mapping in `src/issues/engine.ts`
- [ ] T018 [US2] Integrate automatic issue transitions into `orchestrator.spawn`, `publishEngine.publishPr`, and `orchestrator.dropWorktree` in `src/core/orchestrator.ts`
- [ ] T019 [US2] Register `mannostree issue transition` CLI command in `src/cli/commands/issue.ts`
...
- [ ] T020 [P] [US3] Unit test for evidence comment builder and comment posting in `tests/unit/issue-comment.test.ts`

### Implementation for User Story 3
- [ ] T021 [US3] Implement `postComment` and `syncEvidence` in `src/issues/engine.ts`
- [ ] T022 [US3] Register `mannostree issue comment` and `mannostree issue sync` CLI commands in `src/cli/commands/issue.ts`
...
- [ ] T023 [P] [US4] Unit test for issue drift detection engine in `tests/unit/issue-drift.test.ts`

### Implementation for User Story 4
- [ ] T024 [US4] Implement `checkIssueDrift` and `listIssues` in `src/issues/engine.ts`
- [ ] T025 [US4] Register `mannostree issue status` and `mannostree issue list` CLI commands in `src/cli/commands/issue.ts`
...
- [ ] T026 [P] [US5] Unit test for issue tracker doctor audits in `tests/unit/issue-doctor.test.ts`

### Implementation for User Story 5
- [ ] T027 [US5] Implement `auditIssueTrackers()` diagnostic check in `src/core/doctor.ts`
- [ ] T028 [US5] Update doctor CLI output renderer to format issue tracker diagnostics in `src/cli/output.ts`
...
- [ ] T029 [P] Export all Movement 10 types and issue engines in `src/issues/index.ts` and `src/index.ts`
- [ ] T030 Integrate `IssueSyncEngine` into `MannostreeOrchestrator` in `src/core/orchestrator.ts`
- [ ] T031 Run TypeScript compilation and strict lint checks via `npm run lint`
- [ ] T032 Run full test suite with coverage reporting via `npm test`
- [ ] T033 [P] Update CLI documentation and issue tracker examples in `README.md` and `GEMINI.md`
- [ ] T034 Validate operator workflows per `specs/010-issue-tracker-sync/quickstart.md`
