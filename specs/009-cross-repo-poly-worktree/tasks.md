...
- [x] T001 Extend `MannostreeConfigSchema` in `src/config/schema.ts` and implement `.mannostree.poly.yml` loader/validator in `src/poly/manifest.ts`
...
- [x] T002 [P] Define core types for `PolyManifestConfig`, `PolyRepoMemberConfig`, `PolyLinkRule`, `PolyLinkRecord`, `PolyWorktreeGroupRecord`, and `PolyReleaseManifest` in `src/types/index.ts`
- [x] T003 [P] Implement Zod validation schemas for `PolyManifestConfigSchema`, `PolyLinkRecordSchema`, `PolyWorktreeGroupRecordSchema`, and `PolyReleaseManifestSchema` in `src/metadata/schema.ts`
- [x] T004 Implement `MetadataStore` methods for saving/loading `.mannostree/poly-registry.json` and `.mannostree/poly-links.json` in `src/metadata/store.ts`
- [x] T005 Implement base `PolyEngine` class in `src/poly/engine.ts` with member repository path resolution and pre-flight validation
...
- [x] T006 [P] [US1] Unit test for `PolyEngine.spawn` atomic execution and rollback stack in `tests/unit/poly-spawn.test.ts`
- [x] T007 [P] [US1] Unit test for `PolyEngine.drop` and decommissioning in `tests/unit/poly-drop.test.ts`
- [x] T008 [P] [US1] Integration test for `mannostree poly spawn` and `poly drop` CLI commands in `tests/integration/poly-lifecycle.test.ts`

### Implementation for User Story 1
- [x] T009 [US1] Implement atomic multi-repo `spawnPolyWorktree` with rollback stack in `src/poly/engine.ts`
- [x] T010 [US1] Implement multi-repo `dropPolyWorktree` with safe flag handling (`--discard-uncommitted --yes`) in `src/poly/engine.ts`
- [x] T011 [US1] Register `poly spawn` and `poly drop` CLI commands in `src/cli/commands/poly.ts`
...
- [x] T012 [P] [US2] Unit test for cross-repo package linkers (`npm`, `python`, `go`, `cargo`, `symlink`) in `tests/unit/poly-link.test.ts`

### Implementation for User Story 2
- [x] T013 [US2] Implement link strategies (`npm link`, Python editable `-e`, Go `replace`, Cargo `[patch]`, `symlink`) in `src/poly/link.ts`
- [x] T014 [US2] Implement `linkPolyGroup` and `unlinkPolyGroup` in `src/poly/engine.ts`
- [x] T015 [US2] Register `poly link` and `poly unlink` CLI commands in `src/cli/commands/poly.ts`
...
- [x] T016 [P] [US3] Unit test for `poly sync`, `poly status`, and `poly exec` in `tests/unit/poly-sync-status.test.ts`

### Implementation for User Story 3
- [x] T017 [US3] Implement `syncPolyGroup`, `getPolyStatus`, and `execPolyCommand` in `src/poly/engine.ts`
- [x] T018 [US3] Register `poly sync`, `poly status`, and `poly exec` CLI commands in `src/cli/commands/poly.ts`
...
- [x] T019 [P] [US4] Unit test for multi-host joint poly PR publisher in `tests/unit/poly-publish.test.ts`

### Implementation for User Story 4
- [x] T020 [US4] Implement `PolyPublishEngine.publishPolyPR` and release manifest builder in `src/poly/publish.ts`
- [x] T021 [US4] Register `poly pr` CLI command in `src/cli/commands/poly.ts`
...
- [x] T022 [P] [US5] Unit test for poly-repository health audits in `tests/unit/poly-doctor.test.ts`

### Implementation for User Story 5
- [x] T023 [US5] Implement `auditPolyRepositories()` diagnostic check in `src/core/doctor.ts`
- [x] T024 [US5] Update doctor CLI output renderer to format poly-repository diagnostics in `src/cli/output.ts`
...
- [x] T025 [P] Export all Movement 9 types and poly engines in `src/poly/index.ts` and `src/index.ts`
- [x] T026 Integrate `PolyEngine` and `PolyPublishEngine` into `MannostreeOrchestrator` in `src/core/orchestrator.ts`
- [x] T027 Run TypeScript compilation and strict lint checks via `npm run lint`
- [x] T028 Run full test suite with coverage reporting via `npm test`
- [x] T029 [P] Update CLI documentation and multi-repo examples in `README.md` and `GEMINI.md`
- [x] T030 Validate operator workflows per `specs/009-cross-repo-poly-worktree/quickstart.md`
